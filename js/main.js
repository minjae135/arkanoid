...
function makeBall(stick = true, angle = null) {
...
function resetBalls(stick = true) {
...
function resetGame() {
    ...
    S.setLaserTimer(0);
    S.setShakeAmount(0); // 리셋 시 흔들림 초기화
    S.setCombo(0);
    S.setComboTimer(0);
    S.resetItems();
    S.resetLasers();
...
function applyPaddleWidthByPower() {
...
function applyItem(type) {
    ...
    function handleBrickCollision(brick) {
        ...
        if (brick.type === C.BLOCK_TYPES.EXPLOSIVE) {
            brick.alive = false;
            // S.setScore(S.score + 10); // 위에서 통합 계산함
            playSound('break');
