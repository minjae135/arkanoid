...
const UPGRADES = [
    ...
    { name: "SCORE MASTER", description: "모든 점수 획득량 20% 증가", action: () => { } },
    ...
    { name: "LUCKY DROP", description: "아이템 드롭 확률 증가", action: () => { } }
];
...
function update(dt) {
    if (S.uiModalOpen || upgradeModalOpen)
        return;
    if (I.keys.has('KeyP')) {
        I.keys.delete('KeyP');
...
function render() {
    ...
    D.drawItems(ctx);
    if (S.paused)
        D.drawOverlay(ctx, 'PAUSED', 'TAP TO CONTINUE');
    else if (!S.running)
        D.drawOverlay(ctx, 'GAME OVER', `STAGE: ${currentStage}  •  TAP TO RESET`);
    else if (S.balls.every(b => b.stuck) && !upgradeModalOpen)
        D.drawOverlay(ctx, 'READY?', 'TAP TO LAUNCH');
    ctx.restore();
}
