...
export function handleScoreClick(): void {
    const now = Date.now();
...
function showAdminButtonInControls(): void {
    if (document.getElementById('btn-admin-persistent')) return;
...
function createAdminModalUI(): void {
    ...
    document.getElementById('check-god-mode')?.addEventListener('change', (e) => {
        S.setGodMode((e.target as HTMLInputElement).checked);
    });
    ...
    document.getElementById('input-max-balls')?.addEventListener('change', (e) => {
        ...
        if (!isNaN(val) && val >= 1) { 
            ...
            (e.target as HTMLInputElement).value = String(C.BALL.maxCountDefault);
            syncSlidersWithState();
        }
    });

    // 글로벌 함수 등록 (아이템 즉시 소환용)
    (window as any).applyAdminItem = (type: string) => {
...
function updateAdminUIByRank(): void {
    const rank = S.adminRank;
    const label = document.getElementById('admin-rank-label');
    if (label) label.textContent = `LV.${rank}`;
    
    // UI 상태 동기화
    const checkGodMode = document.getElementById('check-god-mode') as HTMLInputElement;
    if (checkGodMode) checkGodMode.checked = S.godMode;

    const rangeDropRate = document.getElementById('range-drop-rate') as HTMLInputElement;
    const labelDropRate = document.getElementById('label-drop-rate');
    if (rangeDropRate) {
        rangeDropRate.value = String(S.adminDropRate);
        if (labelDropRate) labelDropRate.textContent = S.adminDropRate === -1 ? "기본" : `${Math.round(S.adminDropRate * 100)}%`;
    }

    const rangePaddleSpeed = document.getElementById('range-paddle-speed') as HTMLInputElement;
    const labelPaddleSpeed = document.getElementById('label-paddle-speed');
    if (rangePaddleSpeed) {
        rangePaddleSpeed.value = String(S.adminPaddleSpeedScale);
        if (labelPaddleSpeed) labelPaddleSpeed.textContent = `${S.adminPaddleSpeedScale.toFixed(1)}x`;
    }

    const inputScoreMultiplier = document.getElementById('input-score-multiplier') as HTMLInputElement;
    if (inputScoreMultiplier) inputScoreMultiplier.value = String(S.adminScoreMultiplier);

    const inputMaxBalls = document.getElementById('input-max-balls') as HTMLInputElement;
    if (inputMaxBalls) inputMaxBalls.value = String(S.adminMaxBallCount === -1 ? S.maxBallCount : S.adminMaxBallCount);

    // 권한에 따른 요소 가리기
    const rows = document.querySelectorAll('.admin-row');
    rows.forEach(row => {
        if (row.classList.contains('rank-1') && rank < 1) (row as HTMLElement).style.display = 'none';
        else if (row.classList.contains('rank-2') && rank < 2) (row as HTMLElement).style.display = 'none';
        else if (row.classList.contains('rank-3') && rank < 3) (row as HTMLElement).style.display = 'none';
        else (row as HTMLElement).style.display = 'flex';
    });
}
